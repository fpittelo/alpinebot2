...
4.  **GitHub Secrets:** The _only_ secrets stored in GitHub are the `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `AZURE_SUBSCRIPTION_ID`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. These are needed for the OIDC login to run Terraform. The actual application secrets (DB passwords, API keys) are generated by Terraform and injected directly into Key Vault, never touching the GitHub runner.
