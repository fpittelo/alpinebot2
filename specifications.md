...
### The following is a representation of the logical component diagram using Mermaid syntax:
...
4. **GitHub Secrets:** The *only* secrets stored in GitHub are the `AZURE_CLIENT_ID`, `SUBSCRIPTION_ID`, etc., needed for the OIDC login to run Terraform. The actual application secrets (DB passwords, API keys) are generated by Terraform and injected directly into Key Vault, never touching the GitHub runner.
